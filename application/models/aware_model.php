    ...
    */
    function insert( $database, $table ) {
        ...
        if ( $this->input->post('data') != null ) {
            ...
            # Build an array from the ignore_list.
            # Make sure that ignore_list is placed under the same dir as this PHP file.
            # Consider chmod 644 for the ignore_list to prevent accidental writing.
            # Each line of the ignore_list should be a device_id to be ignored.
            $lines = file(getcwd() . "/ignore_list.txt", FILE_IGNORE_NEW_LINES);

            // $tmplog = fopen("/tmp/ignore_feature.log", "a+") or die("Unable to open file!");

            # Debug the array once it's read from the txt.
            // fwrite($tmplog, print_r($lines, TRUE));
            // fwrite($tmplog, "\n");
            # Debug the current device_id to be processed.
            // fwrite($tmplog, $this->input->post('device_id'));
            // fwrite($tmplog, "\n");
            // fwrite($tmplog, $table);
            // fwrite($tmplog, "\n");

            if(in_array($this->input->post('device_id'), $lines)){
                echo "ignore";
                // fwrite($tmplog, "ignore");
                // fwrite($tmplog, "\n");

                # Technically, this already achives the "ignore" feature.
                # However, the app is still collecting data from the ignored user.
                # Unsubscribe the user from the study when desired.
                # unsubscribe_from_study($this->input->post('device_id'))

            } else {
                echo "prepare to sync";
                // fwrite($tmplog, "prepare to sync");
                // fwrite($tmplog, "\n");
